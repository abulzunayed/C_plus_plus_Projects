...
typedef struct   // or we can declare as struct class:  strurct data{  (/*-----*/)};
{
	int date;
	int month;
	int year;
...
void books::add()
{
        ...
        stmt.str(" ");  // The STR() function returns a number as a string.
        stmt << "Insert into books(name,auth,price,qty) values('" << name << "','" << auth << "'," << price << "," << qty << ");";
        query = stmt.str();  // The STR() function returns a number as a string.
        q = query.c_str();
        mysql_query(conn,q);
...
void books::update_price()
{
        ...
        if((row = mysql_fetch_row(res_set)) != NULL)
        {
            ...
            cin >> choice;
            if (choice == 121 || choice == 89)   // ASCII character arithmathic value, Y= 89, y=121.
            {
...
// ---------------------------------//
//   Member function of Member class
// -----------------------------------//
...
// ---------------------------------//
//   Member function of Sales class
// -----------------------------------//
void sales::add()
{
...
void pass()
{
        ...
        else
        {
            ...
            cout << "Press any key...";
            getch();
            exit(1);  // Both exit(0) and exit(1) are jump statements of C++ and are used to terminate the program.
        }
        return;
}
    ...
    if( conn)
        {
            while (1)
            {
                system( "cls" );   // Here use for clean the screen. Why is system() hated so much? 
                main_menu( );
            }
    ...
    else
        {
            ...
            getch();  // many reason uses getch(), on of reason to hold on the screen until press any key on keyboard
        }
